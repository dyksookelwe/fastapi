...
def post_book(book_info: BookCreate, db: Session = Depends(get_db)):
    existing_book = (
        db.query(Book)
        .where(Book.title == book_info.title, Book.author_id == book_info.author_id)
        .first()
    )
    ...
    category = db.query(Category).where(Category.id == book_info.category_id).first()
    if category is None:
        raise HTTPException(
            status_code=404,
...
def update_book(book_id: int, book_data: BookUpdate, db: Session = Depends(get_db)):
    ...
    if book_data.author_id is not None:
        author = db.query(Author).where(Author.id == book_data.author_id).first()
        if author is None:
            raise HTTPException(
                status_code=404,
                detail="Author is not found"
    ...
    if book_data.category_id is not None:
        category = db.query(Category).where(Category.id == book_data.category_id).first()
        if category is None:
            raise HTTPException(
                status_code=404,
                detail="Category is not found"
